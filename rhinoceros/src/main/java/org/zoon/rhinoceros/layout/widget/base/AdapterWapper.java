...
import org.zoon.rhinoceros.layout.adapter.JsonArrayAdapter;
...
import org.zoon.rhinoceros.utils.GetTask;
...
public class AdapterWapper<T extends AbsListView> extends HView<T> {
    ...
    public void setData(JsonValue value) {
        mAfterReadyUiFuncExecList.add(new UiFuncExec("setData", value) {
            @Override
            protected void onExec(Object[] values) {
                JsonValue value = (JsonValue) values[0];
                if (value.isArray()) {
    ...
    void loadedDataThenBind(String hjson) {
        try {
            hjson = getLayoutInflater().getTemplate().apply(hjson);
        } catch (IOException e) {}
